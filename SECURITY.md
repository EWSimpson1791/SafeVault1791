...
login attempt. These are non-negotiable and must not be weakened in any
future commit without a documented security review.
...
| Failure log | `data/auth_failures.log` | UTC timestamp + reason code on every failure |
...
- The combination of schema + algorithm + any leaked hash gives an attacker
...
Every push or pull request touching `auth/**` or
`tests/test_auth_manager.py` triggers `.github/workflows/auth_security.yml`,
which runs the full 19-test auth security suite on Python 3.12.
...
2. Contact the repository owner directly via GitHub
   (`@EWSimpson1791`) using a private message.
